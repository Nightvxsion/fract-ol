...
*/
static void	my_pixel_put(int x, int y, t_img *img, int color)//Esta función se utiliza para poner un píxel de un determinado color en una imagen 
//Toma cuatro parámetros:
//`int x`: la coordenada horizontal (la posición del píxel en la imagen).
//`int y`: la coordenada vertical (la posición del píxel en la imagen).
//`t_img *img`: un puntero a una estructura `t_img`, que contiene información sobre la imagen, como el puntero a los píxeles y la longitud de línea.
//`int color`: el color que se desea poner en el píxel especificado.
{
	int	offset;

	offset = (y * img->line_len) + (x * (img->bpp / 8));
	//Aquí se está calculando la posición correspondiente en el array de píxeles para el píxel que se quiere modificar.
    //img->line_len` es la longitud de una línea de la imagen en bytes (que incluye el ancho de todos los píxeles en esa línea y posiblemente padding).
    //img->bpp` (bits por píxel) normalmente representa cuántos bits se usan para almacenar el color de un solo píxel. Para calcular el número de bytes por píxel, se divide por 8.
    //La fórmula `(y * img->line_len) + (x * (img->bpp / 8))` calcula el desplazamiento (offset) en bytes desde el inicio de la memoria de píxeles hasta el pixel (`x`, `y`).
	*(unsigned int *)(img->pixels_ptr + offset) = color;
	//En esta línea estamos accediendo a la memoria de píxeles usando el offset calculado anteriormente.
    //img->pixels_ptr` es un puntero que apunta al bloque de memoria donde se almacenan todos los píxeles de la imagen.
    //`+(offset)` se usa para navegar hasta la posición correcta en ese bloque de memoria.
    //Se usa un cast de tipo `(unsigned int *)` porque estamos tratando con datos de color que se representan como valores enteros de 32 bits (4 bytes).
    //Finalmente, se asigna el `color` al píxel en la posición especificada.
}
//La función `my_pixel_put` permite establecer el color de un píxel en una imagen en coordenadas específicas (x, y). Realiza el cálculo necesario para ubicar 
//el lugar correcto en la memoria donde se guardan los datos de los píxeles y luego pone el color deseado en esa posición.
//Si tienes alguna pregunta o deseo de profundizar más sobre alguna parte específica de esta función, ¡no dudes en decírmelo!
...
//a un color entre BLACK y WHITE, y luego se coloca en la imagen. El color va cmbiando en cada iteracion y ese color se poner luego debajo em my_pixel_put
